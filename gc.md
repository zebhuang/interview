...
Python 在垃圾回收中采用了引用计数、部分标记清除和分代垃圾回收。其中以引用计数为主，当对象的引用为0时执行GC。但引用计数无法回收循环引用的问题，所以引入了部分标记清除和分代垃圾回收。分代垃圾回收分为3代，分别为0、1、2代，当某代成员达到阈值时执行部分标记清除GC，在部分标记清除GC中存活下来的对象晋升到更老一代中。之所以采用部分标记清除GC而不是标记清除是因为标记清除目的是为了回收循环引用的垃圾，但一般来说循环引用垃圾应该很少，标记清除GC又会遍历整个堆，所以效率很低。所以采用部分标记清除GC，只对可能有循环引用的对象群指向标记清除GC，也就是部分标记清除GC。标记清除算法是为了查找活动对象，而部分标记清除算法是为了查找非活动对象。

Python的部分标记清除GC的步骤：
- 在可能出现循环引用的容器对象（以下简称对象）生成的时候利用双向链表链接起来
- 将对象的内部的引用计数器复制到内部结构体的字段中
- 对非从根对象引用的对象的复制计数器执行减量（引用计数减一）操作
- 将经过减量操作后复制计数器为0和没有从活动对象对引用的链接到不可达对象链表（双向链表）
- 其他对象连接到可到对象链表（双线链表）
- 对不可达对象链表执行GC回收操作
- GC完毕后把可达对象按原样连接到对象链表
...
| GC算法       | 特点                                                         |
| ------------ | ------------------------------------------------------------ |
| 引用计数     | Python主要GC算法，可即刻回收垃圾，最大暂停时间短，但无法解决循环引用问题 |
| 部分标记清除 | 作用是解决循环引用，但由于标记清除的作用需要遍历整个堆，所以采用针对可能会出现循环引用的对象执行部分标记清除，通过减量和根引用活动对象保留来筛选出循环引用对象 |
| 分代垃圾回收 | 对象存活时间不同，而采取的不同频率的部分标记清除GC。Python中分为3代（容器对象链表），在代中成员满的时候就执行GC，将GC中存活下来的对象晋升。在对年轻代执行部分标记清除时，从老年代引用的对象也像从根引用对象一样不执行减量操作。 |
